...
def run_agent_loop(
    ...
    while True:
        ...
        # content_block_index → current_tool_calls 中的位置
        # event.index 是全局索引（包含 thinking/text 块），需要映射到 tool_calls 列表索引
        _tool_use_index_map: Dict[int, int] = {}
        ...
        # 流式调用 API
        # 思考模式配置：
        # - thinking: 开启思考模式（DeepSeek 默认开启，显式传入确保生效）
        # - output_config.effort: 思考强度，high 适合普通任务，max 适合复杂 Agent 任务
        # - max_tokens: 包含思维链+最终回答的总 token 上限，思考模式需更大空间
        kwargs = {
            "model": state.model,
        ...
        if config.thinking_enabled:
            ...
            kwargs["output_config"] = {"effort": os.environ.get("THINKING_EFFORT", "max")}
            ...
            for event in stream:
                if event.type == "message_start":
                    ...
                    if block.type == "thinking":
                        ...
                        # 记录全局 content_block index 到 tool_calls 列表索引的映射
                        _tool_use_index_map[event.index] = len(current_tool_calls)
                    ...
                    if delta.type == "thinking_delta":
                        ...
                        # 更新工具调用参数
                        # event.index 是全局 content block 索引（可能包含 thinking/text 块在前面）
                        # 需要通过 _tool_use_index_map 映射到 current_tool_calls 的列表索引
                        block_index = getattr(event, 'index', -1)
                        tool_list_index = _tool_use_index_map.get(block_index, -1)
                        if 0 <= tool_list_index < len(current_tool_calls):
                            tool_call = current_tool_calls[tool_list_index]
                            # 累积 JSON 输入 (简化处理)
        ...
        if config.tool_execution_mode == "parallel" and len(tool_calls) > 1:
            # 按并发安全性分组：安全工具可并行，不安全工具必须串行
            safe_calls = []
            unsafe_calls = []
            for tc in tool_calls:
                tool = next((t for t in state.tools if t.name == tc.name), None)
            ...
            # 不安全工具：逐个串行执行（write/edit/bash 需要排他访问）
            for tc in unsafe_calls:
                state.pending_tool_calls.append(tc.id)
                _, tool_result, result_msg = _execute_single_tool(
                    tc, state.tools, state, event_sink)
                tool_results_map[tc.id] = (tool_result, result_msg)

            # 安全工具：并行执行（read/grep/find/ls 只读操作，无副作用）
            if safe_calls:
                with ThreadPoolExecutor(max_workers=len(safe_calls)) as executor:
                    futures = {
                        executor.submit(
                            _execute_single_tool, tc, state.tools, state, event_sink
                        ): tc
                        for tc in safe_calls
                    }
                    for future in as_completed(futures):
                        tc, tool_result, result_msg = future.result()
                        tool_results_map[tc.id] = (tool_result, result_msg)

            # 按原始顺序合并所有结果到一条消息
            # API 要求 assistant 的所有 tool_use 必须在紧接着的下一条消息里全部给出 tool_result
