...
class TerminalRenderer:
    ...
    def __init__(self):
        ...
        self._streaming_text_buffer: str = ""
    ...
    def _on_message_update(self, event: AgentEvent) -> None:
        ...
        if event.delta:
            # 缓冲文本增量，等消息完成后统一渲染
            self._streaming_text_buffer += event.delta
    ...
    def _on_message_end(self, event: AgentEvent) -> None:
        ...
        if event.message:
            ...
            # 渲染文本内容（通过 markdown 渲染器）
            if self._in_streaming_output:
                self._render_text_with_markdown(event.message)
            else:
    ...
    def _on_tool_execution_end(self, event: AgentEvent) -> None:
        ...
        if tool_info:
            ...
            # 显示工具结果
            if event.result:
                if tool_info.tool_name == "todo_write" and event.result.details:
                    self._render_todo_progress(event.result.details)
                else:
    ...
    def _render_text_with_markdown(self, message: Message) -> None:
        """
        将消息中的文本内容通过 markdown 渲染器渲染输出
        ...
        for block in message.content:
            if isinstance(block, TextContent) and block.text:
                ...
                pass  # 工具调用在 tool_execution_start 事件中处理
            elif isinstance(block, ToolResult):
                pass  # 工具结果在 tool_execution_end 事件中处理
    ...
    def _render_todo_progress(self, details: dict) -> None:
        """渲染任务进度面板（支持两级层级和依赖标记）"""
        todos = details.get("new_todos", [])
        completed = details.get("completed", 0)
        total = details.get("total", 0)
        all_done = details.get("all_done", False)

        if not todos and not all_done:
            return
        ...
        print(f"  {Colors.BOLD}任务进度{Colors.RESET}")

        if all_done:
            print(f"  {Colors.GREEN}✓ 全部完成！{Colors.RESET}")
        else:
            for t in todos:
                ...
                dep_str = ""
                if deps:
                    dep_str = f" {Colors.DIM}(依赖: {', '.join(deps)}){Colors.RESET}"

                print(f"  {icon} {Colors.BOLD}{tid}.{Colors.RESET} {content}{dep_str}")

                # 渲染子任务
                children = t.get("children", [])
                for child in children:
                    c_icon = status_icon(child.get("status", "pending"))
                    c_id = child.get("id", "")
                    c_content = child.get("content", "")
                    c_deps = child.get("depends_on", [])

                    c_dep_str = ""
                    if c_deps:
                        c_dep_str = f" {Colors.DIM}(依赖: {', '.join(c_deps)}){Colors.RESET}"

                    print(f"    {c_icon} {c_id}. {c_content}{c_dep_str}")

        # 底部进度
        bar = f"进度: {completed}/{total} 已完成" if not all_done else "进度: 全部完成 ✓"
        print(f"  {Colors.DIM}{'─' * 50}{Colors.RESET}")
