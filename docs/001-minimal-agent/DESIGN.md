...
| 参数 | 类型 | 必填 | 描述 |
|------|------|------|------|
| `file_path` | string | ✅ | 要读取的文件路径（相对于 workspace 目录） |

#### `write` 工具（写入文件）
| 参数 | 类型 | 必填 | 描述 |
|------|------|------|------|
| `file_path` | string | ✅ | 要写入的文件路径（相对于 workspace 目录） |
| `content` | string | ✅ | 要写入文件的内容 |

#### `edit` 工具（编辑文件）
| 参数 | 类型 | 必填 | 描述 |
|------|------|------|------|
| `file_path` | string | ✅ | 要编辑的文件路径（相对于 workspace 目录） |
| `old_string` | string | ✅ | 要查找并替换的精确字符串（必须唯一出现） |
...
**安全限制**：所有操作限制在 `workspace/` 目录下，bash 命令超时时间 300 秒
